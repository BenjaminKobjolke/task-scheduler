...
def handle_set_start_time(
    scheduler: TaskScheduler, cli: CliOutput, task_id_str: str, time_value: str
) -> None:
    """Set or clear start time for a task."""
    try:
        task_id = int(task_id_str)
    except ValueError:
        cli.error(f"Invalid task ID: {task_id_str}")
        sys.exit(1)
    ...
    if time_value.lower() == "none":
        ...
        try:
            ...
            cli.error(
                f"Invalid time format: {time_value}. Use HH:MM format or 'none' to clear."
            )
            sys.exit(1)
    ...
    try:
        ...
        if new_start_time:
            cli.info(
                f"Task '{task['name']}' (ID: {task_id}) start time set to {new_start_time}"
            )
        else:
            cli.info(f"Task '{task['name']}' (ID: {task_id}) start time cleared")
...
def handle_set_interval(
    scheduler: TaskScheduler, cli: CliOutput, task_id_str: str, interval_str: str
) -> None:
...
def handle_set_arguments(
    ...
    cli.info(
        f"Current arguments: {' '.join(task['arguments']) if task['arguments'] else 'None'}"
    )
    cli.info("\nEnter new arguments (press Enter to keep current, type 'none' to clear):")
    cli.info('Example: --source "path/to/source" --target "path/to/target"')
    ...
    try:
        ...
        if new_arguments:
            cli.info(
                f"Task '{task['name']}' (ID: {task_id}) arguments set to: {' '.join(new_arguments)}"
            )
        else:
            cli.info(f"Task '{task['name']}' (ID: {task_id}) arguments cleared")
    except ValueError as e:
        cli.error(str(e))
        sys.exit(1)
