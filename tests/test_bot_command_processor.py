...
import pytest
from bot_commander import BotMessage
...
@pytest.fixture()
def processor(scheduler_mock: MagicMock, config: BotConfig) -> TaskCommandProcessor:
    """Create a TaskCommandProcessor with mocked dependencies."""
    with patch("src.bot.command_processor.Logger"):
        return TaskCommandProcessor(scheduler=scheduler_mock, bot_config=config)
...
class TestHelpCommand:
    ...
    def test_help_returns_help_text(self, processor: TaskCommandProcessor) -> None:
        msg = BotMessage(user_id="user1", text="/help")
        response = processor.handle(msg)
        assert response.text == Messages.HELP

    def test_help_with_extra_args_still_works(
...
class TestListCommand:
    ...
    def test_list_returns_formatted_tasks(
        ...
        tasks = [
            _make_task(task_id=1, name="Backup"),
            _make_task(task_id=2, name="Cleanup"),
        ]
...
class TestRunCommand:
    ...
    def test_run_missing_id(self, processor: TaskCommandProcessor) -> None:
...
class TestAddCommand:
    ...
    def test_add_full_wizard_flow(
        ...
        # Step 0: Start
        response = processor.handle(BotMessage(user_id="user1", text="/add"))
        assert response.text == Messages.WIZARD_ADD_START
        ...
        # Step 2: Name
        response = processor.handle(BotMessage(user_id="user1", text="My Backup"))
    ...
    def test_add_wizard_cancel_at_confirm(
        ...
        processor.handle(BotMessage(user_id="user1", text="backup.py"))
        processor.handle(BotMessage(user_id="user1", text="Task"))
        processor.handle(BotMessage(user_id="user1", text="5"))
        processor.handle(BotMessage(user_id="user1", text="skip"))
        processor.handle(BotMessage(user_id="user1", text="skip"))

        # Say "no" at confirm
        response = processor.handle(BotMessage(user_id="user1", text="no"))
...
class TestEditCommand:
    ...
    def test_edit_invalid_id(self, processor: TaskCommandProcessor) -> None:
        msg = BotMessage(user_id="user1", text="/edit abc")
        response = processor.handle(msg)
        assert response.text == Messages.INVALID_TASK_ID.format(Commands.EDIT)

    def test_edit_missing_id(self, processor: TaskCommandProcessor) -> None:
    ...
    def test_edit_full_wizard_flow(
        ...
        task = _make_task(
            task_id=1, name="Backup", script_path="backup.py", interval=60
        )
        scheduler_mock.list_tasks.return_value = [task]

        # Start edit
        processor.handle(BotMessage(user_id="user1", text="/edit 1"))

        # Skip script_path
        processor.handle(BotMessage(user_id="user1", text="skip"))
        # Change name
        processor.handle(BotMessage(user_id="user1", text="New Backup Name"))
        # Skip interval
        processor.handle(BotMessage(user_id="user1", text="skip"))
        # Skip start_time
        processor.handle(BotMessage(user_id="user1", text="skip"))
    ...
    def test_edit_task_error_returns_error_message(
        ...
        processor.handle(BotMessage(user_id="user1", text="/edit 1"))
        processor.handle(BotMessage(user_id="user1", text="skip"))
        processor.handle(BotMessage(user_id="user1", text="New Name"))
        processor.handle(BotMessage(user_id="user1", text="skip"))
        processor.handle(BotMessage(user_id="user1", text="skip"))
        processor.handle(BotMessage(user_id="user1", text="skip"))
        response = processor.handle(BotMessage(user_id="user1", text="yes"))
...
class TestDeleteCommand:
    ...
    def test_delete_confirm_yes(
        ...
        processor.handle(BotMessage(user_id="user1", text="/delete 1"))
        response = processor.handle(BotMessage(user_id="user1", text="yes"))
    ...
    def test_delete_invalid_id(self, processor: TaskCommandProcessor) -> None:
        msg = BotMessage(user_id="user1", text="/delete abc")
        response = processor.handle(msg)
        assert response.text == Messages.INVALID_TASK_ID.format(Commands.DELETE)

    def test_delete_missing_id(self, processor: TaskCommandProcessor) -> None:
...
class TestCancelCommand:
    ...
    def test_cancel_with_no_conversation(self, processor: TaskCommandProcessor) -> None:
        """Without active conversation, /cancel is treated as unknown command."""
    ...
    def test_cancel_clears_active_conversation(
        ...
        response = processor.handle(BotMessage(user_id="user1", text="backup.py"))
        assert response.text == Messages.UNKNOWN_COMMAND
...
class TestUnknownCommand:
    ...
    def test_unknown_command(self, processor: TaskCommandProcessor) -> None:
        msg = BotMessage(user_id="user1", text="/invalid")
        response = processor.handle(msg)
        assert response.text == Messages.UNKNOWN_COMMAND

    def test_plain_text_is_unknown(self, processor: TaskCommandProcessor) -> None:
        msg = BotMessage(user_id="user1", text="hello world")
        response = processor.handle(msg)
        assert response.text == Messages.UNKNOWN_COMMAND

    def test_empty_text_is_unknown(self, processor: TaskCommandProcessor) -> None:
...
class TestMultipleUsers:
    ...
    def test_separate_conversations_per_user(
        ...
        processor.handle(BotMessage(user_id="user2", text="/add"))

        # User1 continues - should be in their own wizard
        response1 = processor.handle(BotMessage(user_id="user1", text="script1.py"))
        assert response1.text == Messages.WIZARD_ADD_NAME

        # User2 continues independently
        response2 = processor.handle(BotMessage(user_id="user2", text="script2.py"))
    ...
    def test_user1_cancel_does_not_affect_user2(
        ...
        response = processor.handle(BotMessage(user_id="user2", text="script.py"))
        assert response.text == Messages.WIZARD_ADD_NAME
    ...
    def test_user_without_conversation_gets_command_handling(
        ...
        response = processor.handle(BotMessage(user_id="user2", text="/list"))
        assert response.text == Messages.NO_TASKS
